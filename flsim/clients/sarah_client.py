...
class SarahClient(Client):
    ...
    def generate_local_update(
        ...
        if (round_number + 1) % large_cohort_period:
            ...
            local_model, optim, optim_scheduler = self.prepare_for_training(local_model)
            prev_model, prev_optim, prev_scheduler = self.prepare_for_training(
                prev_model
            )
    ...
    def _batch_train(
        ...
        if is_target(self.cfg.optimizer, LocalOptimizerBiLevelConfig):
            optimizer.step(batch_metrics, model, training_batch)
        else:
            optimizer.step()
