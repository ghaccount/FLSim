...
class CDClient(Client):
    ...
    def generate_local_update(
        ...
        init_model = (
            self.global_model
            if self.last_updated_model is None
            else self.last_updated_model
        )
        local_model = deepcopy(init_model)
        ...
        if self.optimizer is None:
            self.optimizer = instantiate(
                self.cfg.optimizer, model=init_model.fl_get_module()
            )
        ...
        local_model, weight = self.train(
            local_model, self.optimizer, self.optimizer_scheduler, metric_reporter
        )
        FLModelParamUtils.subtract_model(
            minuend=init_model.fl_get_module(),
            subtrahend=local_model.fl_get_module(),
            difference=local_model.fl_get_module(),
        )
        # Store previous round model
        self.last_updated_model = deepcopy(local_model)
        return local_model, 1.0
    ...
    def train(
        ...
        return super().train(model, optimizer, optimizer_scheduler, metric_reporter)
    ...
    def _batch_train(
        ...
        optimizer.step(self.global_model)
